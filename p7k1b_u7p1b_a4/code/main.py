...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        model.fit(XBin,yBin)
        print("scikit-L2")
        print("Training error %.3f" % utils.classification_error(model.predict(XBin),yBin))
        print("Validation error %.3f" % utils.classification_error(model.predict(XBinValid), yBinValid))
        print("# nonZeros: %d\n" % (model.coef_ != 0).sum())

        model1 = LogisticRegression(penalty='l1',fit_intercept=False)
        model1.fit(XBin,yBin)
        print("scikit-L1")
        print("Training error %.3f" % utils.classification_error(model1.predict(XBin),yBin))
        print("Validation error %.3f" % utils.classification_error(model1.predict(XBinValid), yBinValid))
        print("# nonZeros: %d" % (model1.coef_ != 0).sum())
        ...
        model.fit(XMulti, yMulti)
        print("scikit-learn-one vs all")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
        
        model = LogisticRegression( C=10000, fit_intercept=False,solver='lbfgs',multi_class = 'multinomial')
        model.fit(XMulti, yMulti)
        print("scikit-learn-Softmax")
        print("Training error %.3f" % utils.classification_error(model.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(model.predict(XMultiValid), yMultiValid))
